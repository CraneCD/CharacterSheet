...
export default function MagicInitiateConfigModal({
    ...
    const handleSave = () => {
        ...
        if (!c1 || !c2) {
            toast.error('Please select two cantrips.');
            return;
        }
        if (c1 === c2) {
            toast.error('Please select two different cantrips.');
            return;
        }
        const s1 = spell1?.trim() || null;
        if (!s1) {
            toast.error('Please select one 1st-level spell.');
            return;
        }
        onSave({
            class: miClass,
    ...
    return (
        <Modal onClose={onClose} ariaLabel="Magic Initiate" contentStyle={{ maxWidth: '480px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
